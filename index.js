...
app.post('/login', async (req, res) => {
  ...
  try {
    ...
    if (userRate.blockedUntil && Date.now() < userRate.blockedUntil) {
      await logAttempt(username, 'neuspješno', timestamp);
    ...
    if (korisnik && await bcrypt.compare(password, korisnik.password)) {
      ...
      userRate.attempts = 0; // Reset attempts on successful login
      await logAttempt(username, 'uspješno', timestamp);
    ...
    if (userRate.attempts >= MAX_ATTEMPTS) {
      userRate.blockedUntil = Date.now() + BLOCK_DURATION; // Block for 1 minute
    }
    await logAttempt(username, 'neuspješno', timestamp);
...
async function logAttempt(username, status, timestamp) {
...
app.get('/nekretnine/top5', async (req, res) => {
  ...
  try {
    ...
    // Filter and sort properties by location and date
